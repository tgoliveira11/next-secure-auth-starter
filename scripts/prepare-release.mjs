...
const LEGACY_INTERNAL_VERSION_PATTERN = /^(0|[1-9]\d*)\.(0|[1-9]\d*)\.(0|[1-9]\d*)-internal$/;
...
function parseCurrentVersion(value) {
  const match = VERSION_PATTERN.exec(value) ?? LEGACY_INTERNAL_VERSION_PATTERN.exec(value);
  if (!match) {
    throw new Error(`Current version must use x.y.z or legacy x.y.z-internal SemVer, received '${value}'`);
  }
  return match.slice(1).map(Number);
}
...
export function compareVersions(left, right) {
  ...
  const b = parseCurrentVersion(right);
...
export function bumpVersion(currentVersion, bump) {
  const [major, minor, patch] = parseCurrentVersion(currentVersion);
...
export function inferReleaseBump(currentVersion, unreleased) {
  ...
  if (/\*\*Breaking:\*\*/i.test(unreleased)) {
    return parseCurrentVersion(currentVersion)[0] === 0 ? "minor" : "major";
  }
...
export function resolveReleaseVersion(currentVersion, releaseSpec, unreleased) {
  ...
  if (compareVersions(normalizedSpec, currentVersion) <= 0) {
    throw new Error(`Explicit release version ${normalizedSpec} must be greater than current ${currentVersion}`);
  }
...
export function prepareRelease({ root, releaseSpec = "", date = new Date().toISOString().slice(0, 10) }) {
  const manifestPaths = MANIFEST_PATHS.map((relativePath) => path.join(root, relativePath));
  const manifests = manifestPaths.map(readJson);
  const currentVersion = manifests[1].version;
  if (manifests.some((manifest) => manifest.version !== currentVersion)) {
    throw new Error("All monorepo package manifests must have the same version before release");
  }
  ...
  manifests.forEach((manifest, index) => {
    ...
    writeFileSync(manifestPaths[index], `${JSON.stringify(manifest, null, 2)}\n`);
  });
