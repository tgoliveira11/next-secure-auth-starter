...
describe("rate limit abstraction", () => {
  ...
  it("supports explicit key modes and default endpoint segments", () => {
    expect(
      buildRateLimitKey({
        operation: "auth.login",
        userId: "user@example.com",
    ...
    ).toBe("rate:auth.login:email:user@example.com:endpoint:default");
  });
  ...
  it("enforceRateLimit throws RateLimitError when blocked", async () => {
    const scope = {
      operation: "auth.forgot_password" as const,
      userId: "user-1",
