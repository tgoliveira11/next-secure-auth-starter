...
describe("sendEmail", () => {
  ...
  describe("console adapter", () => {
    ...
    it("handles malformed recipient domains without logging raw links", async () => {
      vi.stubEnv("NODE_ENV", "development");
      await sendEmail({
        to: "invalid-recipient",
        subject: "Test",
        html: "<p>Hello</p>",
        text: "Hello without links",
      });
      expect(safeLogger.info).toHaveBeenCalledWith(
  ...
  describe("smtp adapter", () => {
    beforeEach(() => {
      ...
      vi.stubEnv("EMAIL_FROM", "Next Secure Auth Starter <noreply@localhost>");
      vi.stubEnv("NODE_ENV", "production");
    });
    ...
    it("delegates to SMTP provider with expected payload", async () => {
      ...
      expect(smtpProvider.sendSmtpEmail).toHaveBeenCalledWith(
        "Next Secure Auth Starter <noreply@localhost>",
        input
      );
    });
