...
describe("webauthn config", () => {
  ...
  it("falls back to NEXTAUTH_URL and default localhost origin", async () => {
    delete process.env.WEBAUTHN_ORIGIN;
    process.env.NEXTAUTH_URL = "http://localhost:3001";
    const { getPrimaryWebAuthnOrigin, getWebAuthnRpId } = await import(
      "@/lib/passkey/webauthn-config"
    );
    expect(getPrimaryWebAuthnOrigin()).toBe("http://localhost:3001");
    delete process.env.WEBAUTHN_RP_ID;
    expect(getWebAuthnRpId()).toBe("localhost");
  });

  it("uses hardcoded localhost fallback when no origins are configured", async () => {
    delete process.env.WEBAUTHN_ORIGIN;
    delete process.env.NEXTAUTH_URL;
    vi.resetModules();
    const { getPrimaryWebAuthnOrigin } = await import("@/lib/passkey/webauthn-config");
    expect(getPrimaryWebAuthnOrigin()).toBe("http://localhost:3001");
  });
});
