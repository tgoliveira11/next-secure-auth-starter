...
describe("microsoft provider config", () => {
  ...
  it("rejects invalid tenant id format", () => {
    vi.stubEnv("AUTH_AZURE_AD_ID", VALID_CLIENT_ID);
    vi.stubEnv("AUTH_AZURE_AD_SECRET", "client-secret");
    vi.stubEnv("AUTH_AZURE_AD_TENANT_ID", "not-a-valid-tenant");

    expect(describeMicrosoftProviderConfigIssue()).toBe("invalid_tenant_id_format");
    expect(readMicrosoftProviderEnv()).toBeNull();
  });

  it("reports missing credentials when only one value is set", () => {
    vi.stubEnv("AUTH_AZURE_AD_ID", VALID_CLIENT_ID);
    vi.stubEnv("AUTH_AZURE_AD_SECRET", "");

    expect(describeMicrosoftProviderConfigIssue()).toBe("missing_credentials");
  });
});
