...
describe("accountSessionService", () => {
  ...
  it("returns false when revoking current session without session id", async () => {
    await expect(accountSessionService.revokeCurrentSession(USER_ID, undefined)).resolves.toEqual({
      revoked: false,
    });
