...
export type StarterAppEnvDefaults = {
  appName: string;
  appSlug: string;
  baseUrl: string;
...
export function buildSecureAuthConfigFromEnv(
  defaults: StarterAppEnvDefaults,
  ...
  const cookieSecureFromEnv = readFirstEnv(env, ["AUTH_COOKIE_SECURE"]);
  const cookieSecure =
    cookieSecureFromEnv === undefined
      ? env.NODE_ENV === "production"
      : readBooleanEnv(env, ["AUTH_COOKIE_SECURE"], false);
  ...
  return {
    ...
    auth: {
      afterLoginPath: readEnv(env, "AUTH_AFTER_LOGIN_PATH") ?? defaults.afterLoginPath ?? "/dashboard",
    ...
    passwordPolicy: {
      ...
      minScore: readNumberEnv(
        env,
        ["AUTH_PASSWORD_MIN_SCORE", "PASSWORD_MIN_SCORE"],
        2,
        { min: 0, max: 4 }
      ),
    },
    sessions: {
      maxAgeSeconds: readNumberEnv(
        env,
